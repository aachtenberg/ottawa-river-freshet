...
def ingest_centrales(payload):
    ...
    # HQ publishes the 'Apport filtre' series twice for some sites (3-28
    # Rapide-7 carries two, with differing values). Duplicate (site_id, time)
    # rows inside one merge-duplicates batch are a cardinality violation
    # ("cannot affect row a second time") and 500 the whole POST. Keep the
    # first series per key -- the value ignore-duplicates used to store.
    seen_inflow = set()
            ...
            if tpd == 'Apport filtré':
                for ts, val in data.items():
                    try:
                        ...
                        continue
                    key = (site_id, normalize_ts(ts))
                    if key in seen_inflow:
                        continue
                    seen_inflow.add(key)
                    inflow_rows.append({
