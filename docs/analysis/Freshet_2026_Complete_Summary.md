...
**Regime change empirically documented**: Flood watch member Contributor A compiled Lac Coulonge yearly peaks 1972–2026 and demonstrated an **18-fold increase in super-flood frequency** between 1972–2016 (1 event in 45 years) and 2017–2026 (4 events in 10 years). The Britannia annual-peak step-location test (Test A) places the sharpest break **exactly at 2017** (+19.3% median shift, decays both directions), aligned with the Bryson refurbishment timeline. This sharp peak-step is the load-bearing original finding and is unchanged. **Test C (added May 2026) adds a separate, differently-shaped finding**: basin-wide annual integrated flow volume at Britannia is up ~+17% post-2017 (Welch t=2.85, p≈0.004), preserved through the regulated chain at Pointe-Calumet (+0.18 m annual mean) and Pointe-Claire (+0.21 m). But the volume step has a **2012–2019 plateau** shape rather than a sharp 2017 break — consistent with a gradual climate ramp intensifying through the 2010s, distinct from the sharp 2017 peak-step. Donald Haines's public claim that "the river is bigger now" is directionally correct (~+17%, not his +33%) and the case file engages it directly. **The two findings answer different questions**: the volume rise is basin-wide, gradual, and likely climate/land-use; the peak distortion is regulated-reach-only, sharp at 2017, and operations-attributable. Both are real; they don't compete. The ORRPB's own April 2026 self-presentation engages with neither, framing flood frequency as cyclical natural variation. See Test C in the Policy and Regulatory Analysis section for the full breakdown.
...
### Test C — annual integrated flow volume (added May 2026)

Donald Haines raised the strongest public climate-side counterclaim in the May 6 CBC-article thread: that **yearly total river-flow volume has increased**, roughly from 30 km³/yr to 40 km³/yr. That is a different metric from annual peak flow and from station precipitation. If the post-2017 regime simply reflects much more water moving through the river every year, the case file has to engage that directly.

**Primary measurement — Britannia integrated discharge.** The local WSC HYDAT extract for **02KF005 Ottawa River at Britannia** was integrated by year from daily discharge (`flow_cms`) with a ≥330-observed-days coverage threshold. Britannia is the *only* WSC gauge in the Ottawa-system extract with flow data spanning the 2017 breakpoint — every other Ottawa-system flow station was discontinued in the 1990s during the operator handoff to OPG/HQ (Chats Falls 1994, Bryson 1994, Coulonge 1993, Maniwaki flow 1926, Portage-du-Fort 1948). HYDAT-based volume integration therefore *cannot* be done at multiple Ottawa-system points simultaneously. Source script: `ingesters/climate-history/annual_volume_test.py`.

| Metric | Pre-2017 | Post-2017 | Shift |
|---|---:|---:|---:|
| Calendar-year volume, 1972–2016 vs 2017–2024 | mean **39.0 km³/yr**, median **38.5** (n=44) | mean **45.6 km³/yr**, median **44.8** (n=8) | mean **+17.0%**, median **+16.3%**, Welch t=+2.85, p≈0.004 |
| Water-year volume, 1972–2016 vs 2017–2024 | mean **39.1 km³/yr**, median **40.3** (n=44) | mean **45.0 km³/yr**, median **44.0** (n=8) | mean **+15.2%**, median **+9.3%**, Welch t=+2.59, p≈0.010 |

**Single-station validity check.** The Britannia / Chats-Falls calendar-year volume ratio over 1960–1994 (the period both gauges were active) had mean **1.043**, median **1.051**, standard deviation **0.047** across 34 years. A stable ratio supports Britannia as a defensible single-station proxy for mid-Ottawa basin output during the period when only it remains.

**Spatial corroboration via downstream level (since flow is impossible).** The two downstream-of-Carillon gauges that *are* level-only but span the breakpoint were tested for annual mean level — a usable proxy for time-integrated throughput at backwater gauges via the stage-discharge relation:

| Gauge | Pre-2017 mean (m) | Post-2017 mean (m) | Shift | Welch t / p |
|---|---:|---:|---:|---:|
| **02OA105 Pointe-Calumet** (Carillon tailwater, 1986–2024) | 22.075 (n=28) | 22.259 (n=7) | **+0.183 m (+0.8%)** | t=+2.62, p≈0.009 |
| **02OA039 Pointe-Claire** (past Beauharnois, 1972–2024) | 21.369 (n=43) | 21.579 (n=8) | **+0.211 m (+1.0%)** | t=+2.65, p≈0.008 |

Both downstream gauges show statistically significant annual-mean shifts of similar magnitude (~+18–21 cm). **The total-throughput signal is preserved through the entire regulated chain and is not attenuated by Beauharnois.** Conservation of mass — extra water arriving at the upstream end of the regulated reach has to come out at the downstream end.

**Climate overlay (cold-season match to Test B).** To match Test B's seasonal frame, cold-season (Nov–Apr) precipitation totals were computed at six ECCC stations with adequate coverage: Barrage Témiscamingue **−6.9%**, Parent **−9.4%**, Maniwaki **+0.2%**, Ottawa CDA **+13.3%**, Rouyn **+6.9%**, Val-d'Or A **−5.8%**; median across stations **−2.8%**. **No coherent cold-season precipitation step.** Annual (calendar-year) precipitation is more wet-side: median **+8.4%** across 5 stations. Some of the post-2017 throughput is therefore associable with annual-precipitation increases, but **roughly half of the +17% volume step has no matching cold-season precipitation forcing.**

**Interpretation — the metric split.** This is the load-bearing finding of Test C, and it cleanly separates the two questions the case file is trying to answer:

| Metric | At Britannia | At Pointe-Calumet | At Pointe-Claire | Pattern |
|---|---|---|---|---|
| **Annual mean volume / level** (total throughput) | +17.0% | +0.18 m (+0.8%) | +0.21 m (+1.0%) | **Basin-wide; preserved through regulation** |
| **AMJ peak level** (freshet shape — see Pointe-Calumet section below) | n/a (peak handled in Test A) | +0.49 m AMJ peak avg, 2016–24 vs prior decades | NOT historically extreme; 1915–44 era had higher peaks | **Concentrates near Carillon; dissipates by Pointe-Claire** |

These are the same gauges and the same data window, evaluated under two different metrics, producing two opposite spatial signatures. The annual-mean signal is consistent with a basin-wide climate/land-use forcing — what conservation of mass demands when more water is in the system. The peak-shape signal is consistent with an operations forcing concentrated at the regulated reach — buffered out by the time water reaches the downstream gauges. **The two signals are not in tension; they are answering different questions.**
...
**Extended verification — 39 consecutive days, added 2026-05-23.** The 13-day window above was constrained by the HQ live-telemetry ingester's start date. The same trigger-condition test computed from ORRPB's public per-location daily data (Hull and Carillon) extends the verification to **39 consecutive days**: Hull dock crossed the 42.61 m servitude on **April 13, 2026** and stayed above through **May 21**; Carillon was above the 40.08 m flood-period ceiling on **all 39 of those days**, with a peak overshoot of **+0.44 m on April 15** (Carillon 40.52 m, Hull 43.31 m). Carillon's lowest level during the entire 39-day window was 40.43 m on May 13 — still 35 cm above the binding ceiling and 81 cm above the permitted floor. The 13-day live-telemetry finding is not a sampling artifact of the ingester's start date; the same pattern holds for the entire 39-day flood-period window. Full method and results in [`docs/reports/2004_Carillon_IWMP_operating_envelope.md`](../reports/2004_Carillon_IWMP_operating_envelope.md) under "Extended verification."

**Independent confirmation of the source document.** The same §2.1 table appears as page 3 of ORFA's published whitepaper *Eight Ways to End the Super Floods* (Feb 2025), filed in this directory at [`docs/reports/2025-02-18_ORFA_8_Ways_extract.md`](../reports/2025-02-18_ORFA_8_Ways_extract.md) (PDF: [`2025-02-18_ORFA_8_Ways.pdf`](../reports/2025-02-18_ORFA_8_Ways.pdf)). Chain of custody from HQ's own 2004 IWMP Summary is therefore unambiguous — the operating envelope is HQ-published, ORFA-republished, and Morin-FB-republished, all carrying the identical numbers.
...
- **Multi-event coverage is still limited.** The 2026 exceedance is now documented for the entire **39-day** flood-period trigger window via ORRPB daily data (closes the small-sample concern within 2026). Whether the same non-compliance pattern occurred in prior freshets requires re-running the ORRPB scrape over earlier years (the public endpoint supports ~16 months per query) or accumulating forward telemetry from the live ingester. ORFA's whitepaper (Feb 2025) reports an analogous 2024-freshet pattern (reservoirs at 50% capacity on April 1 despite record-low snowpack — see [`docs/reports/2025-02-18_ORFA_8_Ways_extract.md`](../reports/2025-02-18_ORFA_8_Ways_extract.md)), so the multi-year picture is likely to corroborate, but the formal multi-event Carillon ceiling test is open.
...
**Note on the "computer model took over the operations" hypothesis.** A community hypothesis sometimes offered for the post-2017 change is that operations were transferred from human dispatchers to an automated optimization model around that time. *Public sources refute this framing.* The MORRO (*Modèle d'Optimisation de la Régulation de la Rivière Outaouais*) decision-support system has been used for real-time operation of the Ottawa basin **since 1981** — two years before the ORRPB tripartite agreement was signed. Hydro-Québec's late-2010s GIS-based scheduling refresh is a tooling modernization layered on top of that existing framework, not a first-time automation. There is no documented year in which "computers replaced humans"; both have been in continuous joint use for ~45 years, and the 2017 freshet behaviour change cannot be attributed to an automation transition that happened 36 years earlier. The case file does not advance the computer-takeover hypothesis as a candidate cause. Full sources and timeline: see [`docs/reports/decision_support_history.md`](../reports/decision_support_history.md). The empirical question — what specifically about Bryson's freshet operating practice changed — remains unanswered by HQ and is the subject of the Exhibit D unanswered question and Exhibit F's documented disclosure failure. **Decisions made by operators using a 1981-era optimization tool are still operator decisions; whether those decisions changed in 2017 is a different question from whether the tooling changed in 2017.**
...
**Reservoir-scale operational envelope — Efford (1975) annual drawdowns cited in Haxton & Chubbuck (2002).** The 35 cm Britannia winter shift above is a gauge-level signature. At the reservoir-scale, the operator's joint Ontario MNR + OPG technical report by Haxton & Chubbuck (2002, [`docs/reports/2002_Haxton_Chubbuck_extract.md`](../reports/2002_Haxton_Chubbuck_extract.md)) cites Efford (1975) annual drawdown figures for each major Ottawa River reservoir:
...
These are *primary-source published* annual drawdown figures from a 2002 government-and-operator joint technical report citing 1975 data. **What they establish is that each reservoir's operational envelope is multi-metre at the annual scale** — the buffer capacity to draw down by metres exists and has been used in documented operating practice.

**Caveat on what these figures do *not* establish.** These are *annual* drawdowns — the total range between each year's high and low. They are *not* directly comparable to the 11–47 cm operating bands the case file documents during the 2026 freshet, which are *short-window measurements during peak inflow* when operators are actively matching outflow to inflow. A reservoir with a 4.1 m annual range can still hold a tight band for thirteen days during peak freshet, because the 13-day band sits *within* the larger annual cycle (winter drawdown → spring refill → peak management → summer descent). The case file does not have modern annual-drawdown figures in its data to compare directly against these 1975 numbers, and should not claim that current annual drawdowns are smaller than historical ones without that comparison.

**What is defensible.** The operational envelope at each Ottawa River reservoir has been documented at 1.5–4.1 m of annual range. The 46 cm spring-flood envelope at Carillon (39.62–40.08 m, see Carillon directive enforcement gap above) sits at the very bottom of this scale — well within the system's documented operational range. **The directive does not require unprecedented drawdown; it requires using a small fraction of the envelope the system was designed and operated for.** That is the historical-precedent claim the case file rests on, and it is supported by the primary-source published figures without requiring a direct annual-vs-freshet-window comparison the data does not yet make possible.
...
1. *Climate alone* — ruled out **for the peak-shape regime change** by Exhibit E. Four independent climate tests (regression, watershed-station precipitation, headwater snowpack trend, Britannia step-change location) collectively show no climate forcing that mathematically requires a sharp 2017 step-change in *peak* basin flow. (Test C separately documents that *annual integrated volume* is up ~+17% post-2017 with a 2010s-plateau shape — a real climate co-driver for total throughput, but not for peak shape.)
...
3. *Independent hydrological shift in the river driving the peaks* — the post-2017 **peak-level** regime change is *spatially concentrated near regulated structures* (Pointe-Calumet AMJ peak step-change near Carillon, no equivalent step at the more distant Pointe-Claire; Britannia annual-peak step-change concentrated at exactly 2017). A purely hydrological shift would distribute more uniformly across gauges. (Test C confirms this metric-specifically: annual *mean* level shifts at Pointe-Calumet and Pointe-Claire are similar in magnitude, but annual *peak* attenuation is sharp — the spatial test only rules out hydrological shift as a cause of the peak distortion, not as a co-driver of total throughput.)
...
- **Within-year shift test (Lac Coulonge gauge specifically):** The post-2017 Lac Coulonge regime change is concentrated in the freshet peak — annual mean is unchanged (+2.6 cm), off-season is unchanged, only the spring peak is shifted (+59 cm). This is consistent with operational behavior change *during peak inflow* at Bryson rather than a hydrological shift that would also move other parts of the Lac Coulonge year. (Test C documents that *downstream of Bryson*, at Britannia and the Carillon-tailwater gauges, annual mean *is* up — the within-year flatness is specific to Lac Coulonge, where Bryson absorbs the off-season basin signal into headpond management. The case-file argument is about how Bryson handles the freshet peak, and that argument is unaffected.)

**Strengthened argument:** the operations indictment does not require winning the climate-attribution debate. Test C confirms a real climate co-driver in the form of basin-wide annual-volume increase (+17%, broad 2010s plateau). But the *peak-shape* step at Lac Coulonge remains sharply pinned to 2017, climate testing cannot reproduce its sharp shape or its spatial concentration near Carillon, and the within-year-shift test localises the operational mechanism to freshet peak management at Bryson. Combined with Exhibit D (a specific operational change happened with documented timing) and Exhibit F (corrective mechanisms have not corrected), the operations-driven hypothesis remains the most parsimonious explanation for the *peak-shape* component of the regime change.
...
### Thread 9: Climate and annual-volume tests — quantitative analysis

The climate alternative was tested five ways. **(1) Cold-season precipitation regression**: pre-2017 (n=44) vs post-2017 (n=10) Ottawa CDA precipitation differed by only +3.7% (not significant). A linear model trained on pre-2017 data predicted post-2017 peaks; actual peaks averaged **+58 cm above prediction**, with super-flood years at +90, +157, +113, and +127 cm. **(2) Per-event amplification regression** (Lac Coulonge peak ~ Britannia annual peak): pre-2017 and post-2017 fits are essentially identical (slope shift +0.012 m per 1000 m³/s, intercept shift -4.8 cm, mean post-2017 residual from pre-line +0.1 cm). The lake responds to inflow as it always has — the +59 cm in mean peak is composition-driven, not amplification. **(3) Watershed-station forcing**: across nine ECCC stations, Apr+May precipitation shifts span -4% to +36% with no coherent step; March snow trends *down* at the upper basin (Témiscamingue -15%, Val-d'Or -14%). **(4) Step location**: median Britannia annual-peak shift maximizes at exactly 2017 (+19.3%) and decays both directions, with three full prior decades (1980s-2000s) at zero super-floods each. **(5) Annual integrated flow volume + spatial corroboration**: Britannia calendar-year volume rises from 39.0 km³/yr pre-2017 to 45.6 km³/yr post-2017 (+17.0%, Welch t=2.85, p≈0.004). The same volume-equivalent shift is preserved through the regulated chain — Pointe-Calumet annual-mean level +0.18 m (p≈0.009), Pointe-Claire +0.21 m (p≈0.008) — so the total-throughput signal is basin-wide as conservation of mass requires. But cold-season (Nov–Apr) precipitation across six ECCC stations has median shift -2.8% (no coherent cold-season step); annual precipitation shifts +8.4% median across five stations, partially explaining the volume rise. The metric split is the load-bearing finding: **annual mean** is preserved through regulation (basin-wide climate/land-use signal), while **AMJ peak** concentrates near Carillon and dissipates by Pointe-Claire (operations signal). The two signals are answering different questions and are additive. **Conclusion:** the river really is carrying ~17% more water (Donald Haines's public claim is directionally right, magnitude smaller than his 30→40 km³/yr framing). That is a real climate/land-use co-driver. But the peak-shape distortion in the regulated reach remains operations-attributable and is a separate finding. **The operator's response is the controllable variable in either frame** — a dam built to manage river flow has an obligation to adapt as inflow patterns change.
