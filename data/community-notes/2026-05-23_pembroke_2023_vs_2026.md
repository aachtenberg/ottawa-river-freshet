...
Dan also notes in the same thread that the only flow gauge below
Des Joachims (just below Westmeath) was decommissioned by OPG in 1993,
and that a 2021 request from Westmeath council to reinstall it was
refused on the grounds that OPG "no longer needed it to run their
operations." That gap is exactly why the audit above can verify
Pembroke levels and Petawawa flow but cannot verify Des Joachims
outflow Q — the public record has the inputs, the operator has the
outputs, and the missing instrument is the one the question hinges on.
...
- **Westmeath flow gauge.** The decommissioning of the
  downstream-of-Pembroke gauge in 1993 — and OPG's 2021 refusal to
  reinstall it on the grounds that they no longer needed it for
  operations — is the load-bearing reason this audit cannot get
  closer to a direct measurement of Pembroke-reach total flow.
